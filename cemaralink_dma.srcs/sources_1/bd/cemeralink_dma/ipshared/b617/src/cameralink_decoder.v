...
assign  axis_lval=dout[66]; 
assign m_axis_tuser=dout[64];
assign m_axis_tlast=dout[65];
 //assign m_axis_tlast =axis_lval_n&&(!fifo_empty);
 assign fifo_rd_en=(!fifo_empty)&&m_axis_tready;

 ///tvaild信号需要变化，仅仅在lval=1才有效
assign m_axis_tvalid=fifo_rd_en&&axis_lval;//FWFT模式下
...
fifo_generator_0 myfifo (
  ...
  .din(data),                  // input wire [66 : 0] din
  .wr_en(cfg_enable_d1),              // input wire wr_en
